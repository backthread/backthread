...
export function buildMcpServer(deps: BuildServerDeps = {}): McpServer {
  ...
  server.registerTool(
    'query',
    {
      ...
      description:
        'Answers why a codebase is the way it is — rationale, trade-offs, superseded/rejected approaches, how a design evolved, and whole-system data-flow — CITED from the repo\'s decision log (the history the code doesn\'t contain). Call it for why/evolution/architecture questions. Before working in an unfamiliar area ("what am I missing about X?", a blindspot pass, unknown unknowns), call it FIRST — a seconds-cheap cited pre-read of what\'s on record (trade-offs knowingly accepted, standing assumptions, known limitations, rejected approaches) — then do your own pass through the code: the pre-read aims your deep dive, it doesn\'t replace it. For a whole-feature \'how does X work\', pair it with reading the code for local mechanics. Do NOT call it for what one function or file does now — read that source. Pass the question; relay verbatim. Read-only.',
      inputSchema: {
        question: z
          .string()
          .optional()
