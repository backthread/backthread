...
export function listSourceFiles(root: string, lang: SourceLang): string[] {
  ...
  const excludes = new Set<string>(
    lang === 'python' ? PYTHON_EXCLUDE_DIRS : lang === 'ruby' ? RUBY_EXCLUDE_DIRS : EXCLUDE_DIRS,
  );
  const out: string[] = [];
...
function countSources(root: string, cap = 4000): { ts: number; python: number; ruby: number } {
  let ts = 0;
  let python = 0;
  let ruby = 0;
  const absRoot = resolve(root);
  ...
  walk(absRoot);
  return { ts, python, ruby };
}

/**
 * Pick the structural adapter language for `repoDir`. TS is the DEFAULT (the
 * pipeline's home turf); Python / Ruby are selected only when the repo is
 * unambiguously that language. A single manifest with no competing manifest
 * decides cheaply (a Python-only manifest → Python, a Ruby-only manifest → Ruby);
 * a repo with several (a Rails app carries both a Gemfile and a package.json for
 * its JS bundler) or none falls back to a source-file count, so the DOMINANT
 * language wins — a Ruby-heavy Rails app extracts as Ruby even with a small JS
 * footprint.
 */
export function detectRepoLanguage(repoDir: string): SourceLang {
  ...
  if (hasPy && !hasTs && !hasRuby) return 'python';
  if (hasTs && !hasPy && !hasRuby) return 'ts';
  if (hasRuby && !hasTs && !hasPy) return 'ruby';
  return pickDominant(countSources(repoDir));
}

/**
 * The language with the most source files, ties resolving to the earliest of
 * [ts, python, ruby] — so `ts` stays the default when nothing is present
 * (byte-identical to the pre-Ruby `python > ts ? 'python' : 'ts'`).
 */
function pickDominant(counts: { ts: number; python: number; ruby: number }): SourceLang {
...
export function detectRepoLanguages(repoDir: string): SourceLang[] {
  const { ts, python, ruby } = countSources(repoDir);
  ...
  const max = Math.max(ts, python, ruby);
